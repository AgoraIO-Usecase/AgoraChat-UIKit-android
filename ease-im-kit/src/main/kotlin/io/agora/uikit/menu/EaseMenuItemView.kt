...
class EaseMenuItemView @JvmOverloads constructor(
    ...
    private fun initAttrs(context: Context, attrs: AttributeSet?, defStyleAttr: Int) {
        val a = context.obtainStyledAttributes(attrs, R.styleable.EaseMenuItemView, defStyleAttr, 0)
        orientation = MenuOrientation.values()[a.getInt(R.styleable.EaseMenuItemView_menuOrientation, 0)]
        setMenuOrientation(orientation)
        gravity = MenuGravity.values()[a.getInt(R.styleable.EaseMenuItemView_contentGravity, 0)]
    ...
    fun setMenuOrientation(orientation: MenuOrientation?) {
        ...
        root?.let {
            this@EaseMenuItemView.removeView(root)
        }
        when(orientation) {
            MenuOrientation.HORIZONTAL -> {
                root = LayoutInflater.from(context).inflate(R.layout.ease_chat_menu_item_horizontal, this, false)
                this@EaseMenuItemView.addView(root)
            }
            MenuOrientation.VERTICAL -> {
                root = LayoutInflater.from(context).inflate(R.layout.ease_chat_menu_item, this, false)
                this@EaseMenuItemView.addView(root)
            }
        }
        setMenuGravity(gravity)
