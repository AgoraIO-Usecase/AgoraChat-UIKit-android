...
import io.agora.uikit.common.extensions.keepScreenOn
import io.agora.uikit.databinding.EaseChatVoiceRecorderBinding
...
import io.agora.uikit.common.helper.EaseTimerHelper
import io.agora.uikit.common.helper.EaseVoiceRecorder
import io.agora.uikit.interfaces.OnVoiceRecorderClickListener

class EaseChatVoiceRecorderDialog(
    private val context: Context,
    private val conversationId: String?,
    private val maxRecordSeconds: Int = 60
): EaseBaseSheetFragmentDialog<EaseChatVoiceRecorderBinding>() {
    ...
    private val voiceRecorder by lazy { EaseVoiceRecorder(context, micImageHandler) }
    private val voicePlayer by lazy { EaseChatRowVoicePlayer.getInstance(context) }
    private val timer by lazy { EaseTimerHelper(targetSeconds = maxRecordSeconds) }
    // The record seconds.
    private var recordSeconds: Int = 0
    override fun getViewBinding(
        inflater: LayoutInflater,
        container: ViewGroup?
    ): EaseChatVoiceRecorderBinding? {
        return EaseChatVoiceRecorderBinding.inflate(inflater, container, false)
    }
    ...
    override fun initListener() {
        ...
        timer.setOnTimerListener(object : EaseTimerHelper.OnTimerListener {
            ...
            override fun onTimer(seconds: Int, delayMillis: Long) {
                ...
                binding?.let {
                    ...
                    it.root.post {
                        it.tvVoiceLength.text = context.getString(R.string.ease_chat_record_voice_time, seconds)
                        if (maxRecordSeconds - seconds <= 10) {
                            it.tvVoiceRemainTip.text = context.getString(R.string.ease_chat_record_voice_remain, maxRecordSeconds - seconds)
                        }
                    }
                }
            }
    ...
    private fun setRecordIdleStatus() {
        ...
        setViewsGone()
        binding?.run {
            layoutActionVoiceRecord.setImageResource(R.drawable.ease_chat_voice_icon)
            tvVoiceTip.visibility = View.VISIBLE
            tvVoiceTip.text= context.getString(R.string.ease_chat_record_voice_record)
        }
    }
    ...
    private fun setRecordingStatus() {
        ...
        binding?.run {
            ...
            tvVoiceLength.text = context.getString(R.string.ease_chat_record_voice_time, 0)
            tvVoiceTip.visibility = View.VISIBLE
            tvVoiceTip.text = context.getString(R.string.ease_chat_record_voice_recording)
            voiceWave.startPlay()
        }
    }
    ...
    private fun setRecordStopStatus() {
        ...
        binding?.run {
            ...
            tvVoiceLength.visibility = View.VISIBLE
            tvVoiceTip.visibility = View.VISIBLE
            tvVoiceTip.text = context.getString(R.string.ease_chat_record_voice_play)
            voiceWave.stopPlay()
        }
    }
    ...
    private fun setRecordPlayingStatus() {
        ...
        binding?.run {
            ...
            tvVoiceLength.text = context.getString(R.string.ease_chat_record_voice_time, 0)
            tvVoiceTip.visibility = View.VISIBLE
            tvVoiceTip.text = context.getString(R.string.ease_chat_record_voice_playing)
            voiceWave.startPlay()
        }
    }
    ...
    private fun setPlayerStopStatus() {
        ...
        binding?.run {
            tvVoiceLength.visibility = View.VISIBLE
            tvVoiceLength.text = context.getString(R.string.ease_chat_record_voice_time, recordSeconds)
        }
    }
