package io.agora.uikit.feature.chat.translation
...
class EaseChatMessageTranslationView @JvmOverloads constructor(
    ...
    private fun initAttrs(context: Context, attrs: AttributeSet?) {
        context.obtainStyledAttributes(attrs, R.styleable.EaseChatMessageTranslationView).let { a ->
            if (this.isSender == null){
                isSender = a.getBoolean(R.styleable.EaseChatMessageTranslationView_ease_chat_message_translation_is_sender, false)
            }
            isHistory = a.getBoolean(R.styleable.EaseChatMessageTranslationView_ease_chat_message_translation_is_history, false)
            a.recycle()
        }

        isSender?.let {
            val colorId = if (it) {
                inflate(context, R.layout.ease_row_sent_translation_layout, this)
                R.color.ease_color_text_special_higher
            } else {
                inflate(context, R.layout.ease_row_received_translation_layout, this)
    ...
    fun updateMessageInfo(message: ChatMessage?): Boolean {
        ...
        if (message.ext().containsKey(EaseConstant.TRANSLATION_STATUS)){
            isTranslation = message.getBooleanAttribute(EaseConstant.TRANSLATION_STATUS,false)
