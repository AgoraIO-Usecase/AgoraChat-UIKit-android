...
class EaseWaveView @JvmOverloads constructor(
    ...
    private fun initAttrs(context: Context, attrs: AttributeSet?) {
        val typedArray = context.obtainStyledAttributes(attrs, R.styleable.EaseWaveView)
        typedArray?.run {
            getColor(R.styleable.EaseWaveView_ease_wave_color, -1).let {
                if (it != -1) mPaint?.color = it
            }
            getResourceId(R.styleable.EaseWaveView_ease_wave_color, -1).let {
                if (it != -1) mPaint?.color = ContextCompat.getColor(context, it)
            }
            getDimension(R.styleable.EaseWaveView_ease_wave_width, -1f).let {
                if (it != -1f) originalW = it.toInt()
            }
            getDimension(R.styleable.EaseWaveView_ease_wave_height, -1f).let {
                if (it != -1f) originalH = it.toInt()
            }
            getDimension(R.styleable.EaseWaveView_ease_wave_radius, -1f).let {
                if (it != -1f) {
                    rx = it
                    ry = rx
                }
            }
            getDimension(R.styleable.EaseWaveView_ease_wave_stroke_width, -1f).let {
                if (it != -1f) waveWidth = it.toInt()
            }
            getDimension(R.styleable.EaseWaveView_ease_wave_move_distance, -1f).let {
                if (it != -1f) translationLength = it.toInt()
            }
            getFloat(R.styleable.EaseWaveView_ease_wave_start_alpha, -1f).let {
                if (it != -1f) {
                    mPaint?.alpha = (it * 255).toInt()
                    waveStartAlpha = it
                }
            }
            getFloat(R.styleable.EaseWaveView_ease_wave_end_alpha, -1f).let {
                if (it != -1f) waveEndAlpha = it
            }
            getInt(R.styleable.EaseWaveView_ease_wave_duration, -1).let {
                if (it != -1) duration = it.toLong()
            }
            getBoolean(R.styleable.EaseWaveView_ease_wave_show_bg_wave, false).let {
                showBgWave = it
            }
            getDimension(R.styleable.EaseWaveView_ease_wave_bg_height, -1f).let {
                if (it != -1f) bgWaveH = it.toInt()
            }
            getDimension(R.styleable.EaseWaveView_ease_wave_bg_width, -1f).let {
                if (it != -1f) bgWaveW = it.toInt()
            }
            getDimension(R.styleable.EaseWaveView_ease_wave_bg_radius, -1f).let {
                if (it != -1f) bgWaveR = it.toInt()
            }
            getDimension(R.styleable.EaseWaveView_ease_wave_bg_stroke_width, -1f).let {
                if (it != -1f) bgWaveStrokeW = it.toInt()
            }
            getColor(R.styleable.EaseWaveView_ease_wave_bg_color, -1).let {
                if (it != -1) bgWaveColor = it
            }
            getResourceId(R.styleable.EaseWaveView_ease_wave_bg_color, -1).let {
    ...
    private fun init(context: Context) {
        mPaint?.run {
            color = ContextCompat.getColor(context, io.agora.uikit.R.color.ease_color_primary)
    ...
    private fun drawBgWave(canvas: Canvas) {
        ...
        if (bgWaveColor == -1) {
            bgWaveColor = ContextCompat.getColor(context, io.agora.uikit.R.color.ease_primary_90)
        }
