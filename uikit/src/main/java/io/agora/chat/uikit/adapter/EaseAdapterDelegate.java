...
/**
 * （1）作为代理类的基类，通过{@link com.hyphenate.easeui.adapter.EaseAdapterDelegatesManager}实现与{@link RecyclerView.Adapter}的关联。
 * （2）判断是否使用这个代理类的方法为{@link #isForViewType(Object, int)}，如果它返回true就代表这个代理类进行数据处理。
 * 具体逻辑，可见{@link com.hyphenate.easeui.adapter.EaseAdapterDelegatesManager#getItemViewType(Object, int)}。
 ...
 * 同一个delegate对应多个tag的情况，并对{@link com.hyphenate.easeui.adapter.EaseAdapterDelegatesManager#getItemViewType(Object, int)}中相关逻辑进行修改。
 *
 * @param <T>
 * @param <VH>
 */
